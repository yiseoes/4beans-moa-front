...
const ERROR_MESSAGES = {
    ...
    'BILLING_KEY_NOT_FOUND': '등록된 결제 수단을 찾을 수 없습니다. 카드를 다시 등록해주세요',
    ...
    'LEADER_CANNOT_LEAVE': '파티장은 탈퇴할 수 없습니다. 파티를 삭제하거나 리더를 변경해주세요',
...
const getErrorSeverity = (errorCode, statusCode) => {
    ...
    if (statusCode >= 500 || errorCode?.includes('500') || errorCode?.includes('999')) {
...
export const handleApiError = (error, options = {}) => {
    ...
    const userMessage = customMessage || ERROR_MESSAGES[errorCode] || errorMessage || '오류가 발생했습니다';
    ...
    const errorObj = {
        ...
        severity,
        statusCode,
        canRetry: statusCode >= 500 || errorCode?.includes('500') || errorCode?.includes('NETWORK'),
        needsSupport: errorCode === 'PAY502' || severity === ERROR_SEVERITY.CRITICAL,
    };
...
export const handlePaymentError = (error) => {
    ...
    } else if (errorObj.code === 'PAY404' || errorObj.code === 'BILLING_KEY_NOT_FOUND') {
...
export const handlePartyError = (error) => {
    ...
    if (errorObj.code === 'P409' || errorObj.code === 'ALREADY_JOINED' || errorObj.code === 'PARTY_FULL') {
