...
// Period Filter Component (always white theme)
...
// Enhanced Stat Card with Trend (always white theme)
const StatCard = ({ icon: Icon, title, value, subtitle, color, trend, delay = 0 }) => {
    return (
        <motion.div
            ...
            className="group relative bg-white rounded-2xl border border-gray-100 hover:border-gray-200 p-5 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden"
        >
            <div
...
// Quick Action Button (always white theme)
...
export default function AdminDashboardPage() {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-50 flex justify-center items-center relative z-10">
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
    ...
    const revenueLineOptions = {
        ...
        xaxis: { categories: filteredRevenueData.dates },
    ...
    const partyDonutOptions = {
        ...
        plotOptions: { pie: { donut: { size: "70%", labels: { show: true, total: { show: true, label: "전체", formatter: () => safeStats.totalPartyCount || 0 } } } } },
        dataLabels: { enabled: false },
    };
    ...
    const goalPercentage = Math.min(100, Math.round(((safeStats.thisMonthRevenue || 0) / 10000000) * 100));
        ...
        <div className="min-h-screen bg-slate-50 relative z-10">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="pl-10 pr-4 py-2.5 border border-gray-200 bg-white text-gray-900 rounded-xl text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-[#635bff]/20 focus:border-[#635bff]"
                                />
                            </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <StatCard icon={UserPlus} title="오늘 가입" value={`${safeStats.todayNewUsers || 0}명`} color="#ec4899" trend={-3} delay={0.7} />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            {(safeStats.recentUsers || []).slice(0, 4).map((user, index) => (
                                <motion.div
                                    key={user.odUserId}
                                    initial={{ opacity: 0, x: -20 }}
                                    animate={{ opacity: 1, x: 0 }}
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            {(safeStats.recentPayments || []).slice(0, 4).map((payment, index) => (
                            ...
                            {(!safeStats.recentPayments || safeStats.recentPayments.length === 0) && (
                                <p className="text-sm text-gray-400 text-center py-4">데이터 없음</p>
                            )}
                        </div>
                    </motion.div>
